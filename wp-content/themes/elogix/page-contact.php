...
if(isset($_POST['submitted'])) {
	if(trim($_POST['contactName']) === '') {
		$nameError = __('Lütfen isminizi giriniz.', 'framework');
		$hasError = true;
	} else {
	...
	if(trim($_POST['email']) === '')  {
		$emailError = __('Lütfen geçerli bir email adresi giriniz.', 'framework');
		$hasError = true;
	} else if (!eregi("^[A-Z0-9._%-]+@[A-Z0-9._%-]+\.[A-Z]{2,4}$", trim($_POST['email']))) {
		$emailError = __('Geçersiz email adresi.', 'framework');
		$hasError = true;
	} else {
	...
	if(trim($_POST['comments']) === '') {
		$commentError = __('Lütfen mesajınızı giriniz.', 'framework');
		$hasError = true;
	} else {
		...
		<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
			...
			<div id="post-<?php the_ID(); ?>" class="post">
				...
				<?php if(isset($emailSent) && $emailSent == true) { ?>
								<h3><?php _e('Mesajınız alınmıştır. En kısa sürede sizinle iletişime geçilecektir.', 'framework'); ?></h3>
						<?php } else { ?>
						...
						<form action="<?php the_permalink(); ?>" id="contactform" method="post">
							<p>
								...
								<?php } else { ?>
									<label for="contactName"><?php _e('Isim', 'framework'); ?></label>
								<?php } ?>
							</p>
							<p>
							...
							<p>
								<input type="submit" id="submit" value="<?php _e('Gönder', 'framework') ?>" />
							</p>
						</ul>
