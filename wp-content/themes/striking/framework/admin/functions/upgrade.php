...
class upgradeHelper {
	...
	function init(){
		...
		if (theme_get_option('advance','update_notification') ){
	...
	function download_package_http_response_filter( $response, $args, $url ){
		if ( 0 === strpos( $url, $this->getPackage() ) ) {
			switch (wp_remote_retrieve_response_code( $response )){
				case '401':
					return new WP_Error( 'unauthorized',  __('Please input your purchase code'));
				case '406':
					return new WP_Error( 'not_acceptable',  __('Please contact theme author'));	
			}
	...
	function notices(){
		...
		if($has_update){
			if ( current_user_can('update_core') ) {
				$msg = sprintf( __('%2$s %1$s is available! <a href="%3$s">Please update now</a>.'), $has_update, THEME_NAME, admin_url( 'admin.php?page=theme_advance#update') );
			} else {
	...
	function getPackage(){
		$latest_version = get_transient(THEME_SLUG.'_update');
		$url = 'http://api.kaptinlin.com/themes/'.THEME_SLUG.'/verify-purchase/'.theme_get_option('advance','item_purchase_code').'/upgrade/from/'.THEME_VERSION.'/to/'.$latest_version.'.zip';
		$url = apply_filters('theme_package_url', $url,THEME_SLUG,THEME_VERSION, $latest_version,theme_get_option('advance','item_purchase_code'));
		return $url;
	}
...
if(isset($_GET['action']) && $_GET['action'] == THEME_SLUG.'_theme_update'){
	...
	class Kaptinlin_Theme_Upgrader extends WP_Upgrader {
		...
		function upgrade($theme) {
			...
			$working_dir =  trailingslashit($this->unpack_package( $download ));
