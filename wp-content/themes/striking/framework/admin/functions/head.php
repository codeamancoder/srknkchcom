...
function theme_admin_register_script(){
	...
	wp_register_script('chosen',THEME_ADMIN_ASSETS_URI . '/js/chosen.jquery.js',array('jquery'),'0.91');
	...
	wp_register_script('theme-docs', THEME_ADMIN_ASSETS_URI . '/js/docs.js', array('jquery','jquery-ui-tabs'));
	wp_register_script('theme-portfolio-metabox', THEME_ADMIN_ASSETS_URI . '/js/portfolio.js', array('jquery'));
}
...
function theme_admin_add_script() {
	...
	if (theme_is_post_type('portfolio')){
		wp_enqueue_script('theme-portfolio-metabox');
	}
}
