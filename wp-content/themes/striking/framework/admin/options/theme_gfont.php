<?php
if(!function_exists('theme_gfont_getfonts')){
	function theme_gfont_getfonts(){
		return array(
			"Abel"=>"Abel",
	...
	function theme_gfont_getfontname($font){
		$fonts = theme_gfont_getfonts();
...
if(isset($_GET['page']) && $_GET['page']=='theme_gfont'){
	if (! function_exists("theme_gfont_add_head")){
		function theme_gfont_add_head(){
			...
			$scripts .= "jQuery('[name=\"default_font\"]').live('change',function(){if(jQuery(this).is(':checked')){jQuery(this).parents('tr').siblings().find('input:checked').attr('checked', false).trigger('change');}});";
			$scripts .= "ZeroClipboard.setMoviePath('".THEME_ADMIN_ASSETS_URI."/js/ZeroClipboard.swf');\njQuery('.gfont_font_name').each(function(){\nvar clip = new ZeroClipboard.Client();\nclip.setText(jQuery(this).text());\nclip.glue(this,jQuery(this).parent('.font_name_wrap')[0]);\n});";
			echo $scripts."});\n</script>";
		}
		add_filter('admin_head', 'theme_gfont_add_head');
	}
}
if(!function_exists('theme_gfont_enabled_fonts_option')){
	function theme_gfont_enabled_fonts_option($option, $default){
		$fonts =  theme_get_option('gfont','used_gfont');
		echo '<tr colspan="2"><td style="padding:0"><table class="widefat gfont_table" cellspacing="0"><tbody>';
		$count = 1;
		foreach($fonts as $font){
			if($font == $default){
			...
			echo '<tr><td style="width:15%"><div class="font_name_wrap" style="position: relative;"><a class="gfont_font_name" href="#" title="'.$font.'">'.theme_gfont_getfontname($font).'</a></div></td><td style="width:10%"><input type="checkbox" name="default_font" class="toggle-button" value="'.$font.'"'.$checked.'/></td><td><span class="gfont_preview" id="preview-'.$count.'">This is a preview of the <span style="color:  #379BFF;">'.theme_gfont_getfontname($font).'</span> font. Some numbers: 0123456789 &amp; so on..</span></td></tr>';
			$count ++;
		}
...
$options = array(
	array(
		"name" => __("Google Fonts","striking_admin"),
		"type" => "title"
	),
	array(
		"name" => __("Google Fonts General","striking_admin"),
		"type" => "start"
	),
		array(
			"name" => __("Google Fonts Custom CSS","striking_admin"),
		...
		array(
			"id" => "used_gfont",
			"name" =>__('Enable Google Fonts',"striking_admin"),
			...
			"chosen" => "true",
			"type" => "multiselect",
			"options"=> theme_gfont_getfonts(),
		),
	array(
		"type" => "end"
	),
);
if($fonts =  theme_get_option('gfont','used_gfont')){
	$options[] = array(
		"name" => "Enabled Google fonts list (select a default font)",
		"type" => "start"
	);
	$options[] = array(
		"id" => "default_font",
		"layout" => false,
		"function" => "theme_gfont_enabled_fonts_option",
		"default" => '',
		"type" => "custom"
	);
	$options[] = array(
